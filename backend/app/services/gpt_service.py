...
def _build_criteria_section(criteria: list[dict]) -> str:
    ...
    for i, c in enumerate(criteria, 1):
        ...
        if c.get("description"):
            lines.append(f"   ОПРЕДЕЛЕНИЕ: {c['description']}")
        ...
        lines.append(f"   ⚠️ Ставь 0.5 ТОЛЬКО если выполнена БОЛЬШАЯ ЧАСТЬ критерия. Если выполнена одна мелочь, а главное упущено — ставь 0.")
    return "\n".join(lines)
...
ВАЖНО: Оценивай ТОЛЬКО ДЕЙСТВИЯ МЕНЕДЖЕРА. Действия клиента не влияют на оценку менеджера.

ВАЖНО: Не давай частичный балл (0.5) за действия, не относящиеся к критерию.
- Если менеджер не представился, а просто назвал причину звонка — это НЕ приветствие, ставь 0.
- Если менеджер не закрыл сделку, а просто попрощался — это НЕ закрытие, ставь 0.
- Оценивай строго по описанию критерия, не домысливай.
...
{{
  ...
  "criteria_details": {{
    {', '.join(f'"{c["key"]}": "конкретное пояснение: что менеджер сделал или НЕ сделал (1-2 предложения на русском). Если 1 — что именно хорошо. Если 0.5 — что сделано, а что нет. Если 0 — что конкретно упущено и как это проявилось в диалоге."' for c in criteria)}
  }},
  ...
  "strengths": ["конкретные сильные стороны с примерами из диалога"],
  "growth_areas": ["НЕ НУМЕРУЙ рекомендации цифрами. Каждая — одно предложение с конкретным советом: что не так + как исправить + пример фразы."],
